...
25. **Planar departure trigger emitted after unregistering the Plane:** “planeswalk away from” abilities must observe a semantic departure event while the face-up planar object's command-zone triggers are still registered. Emit departure events for every face-up Plane or Phenomenon before unregistering and bottoming those objects; do not approximate departure by watching the destination-only `PLANESWALKED` event.
