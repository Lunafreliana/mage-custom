...
25. **“Other players” assigned to the source controller without reading its antecedent:** in sequential text, “other players” can mean the complement of the player group identified by the previous sentence, not necessarily every player except “you.” For example, after “each player who controls their commander” performs an action, “other players” means the players who do not control their commander. Preserve that grouping explicitly when resolving the later instruction.
26. **Dynamically granted triggered keyword is installed after its enabling event:** some keyword abilities, such as miracle, depend on a watcher that observes the event which makes a card eligible. XMage emits `DREW_CARD` immediately after moving the card to its owner's hand and before recalculating continuous effects, so granting miracle only to cards currently in hand misses the drawn card. Use a source-scoped watcher that installs the granted ability while handling the enabling event, before triggered abilities are checked. Do not grant the ability to cards in the library as a timing workaround; that changes their characteristics in the wrong zone and the gained ability is cleared by the zone change anyway.
