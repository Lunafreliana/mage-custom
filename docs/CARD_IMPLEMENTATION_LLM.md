...
26. **Dynamically granted triggered keyword lacks its watcher before the event:** some keyword abilities, such as miracle, depend on a watcher that observes the event which makes the card eligible. Adding the keyword to cards already in a hidden zone may not register its watcher early enough for a newly arriving card. When a continuous effect grants such a keyword, also attach the keyword's required game watcher to the granting static ability so it is active before the event.
