...
26. **Turn history starts only when the payoff enters:** a permanent can refer to cards or actions from earlier in the turn. Record that history in a default game watcher from the underlying rules action, rather than attaching the watcher only to the payoff card; otherwise a permanent entering later will incorrectly miss qualifying events.
