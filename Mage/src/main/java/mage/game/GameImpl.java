...
public abstract class GameImpl implements Game {
    ...
    @Override
    public boolean chooseNextPlane(PlaneswalkContext context, int planeCount) {
        UUID playerId = context == null ? null : context.getPlaneswalkingPlayerId();
        Player player = getPlayer(playerId);
        SharedPlanarDeck deck = getPlanarDeckForPlayer(playerId);
        ...
        planes.forEach(plane -> choice.getChoices().add(plane.getName()));
        player.choose(Outcome.Benefit, choice, this);
        PlanarCard chosen = planes.stream()
                .filter(plane -> plane.getName().equals(choice.getChoice()))
                .findFirst()
                .orElse(planes.get(0));
