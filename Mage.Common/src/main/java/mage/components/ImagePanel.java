...
public class ImagePanel extends JPanel {
    ...
    /** Optional BufferedImage property used by special backgrounds that need custom layout. */
    public static final String IMAGE_LAYOUT_PROPERTY = "xmage.imagePanel.layout";
    public static final String IMAGE_LAYOUT_STRETCH = "stretch";
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (IMAGE_LAYOUT_STRETCH.equals(image.getProperty(IMAGE_LAYOUT_PROPERTY))) {
            drawStretch(g);
            return;
        }
    ...
    private void drawStretch(Graphics g) {
        Dimension d = getSize();
        Graphics2D graphics = (Graphics2D) g.create();
        try {
            graphics.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
            graphics.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
            graphics.drawImage(image, 0, 0, d.width, d.height, this);
        } finally {
            graphics.dispose();
        }
    }
