...
public class GrandOssuaryTest extends CardTestPlayerBase {
    ...
    public void deadCreaturesControllerDistributesCountersUsingLastKnownPower() {
        ...
        addTarget(playerB, "Grizzly Bears^X=3");

        setStrictChooseMode(true);
        setStopAt(1, PhaseStep.POSTCOMBAT_MAIN);
        execute();

        assertCounterCount(playerB, "Grizzly Bears", CounterType.P1P1, 3);
        assertCounterCount(playerA, "Memnite", CounterType.P1P1, 0);
    }

    @Test
    public void chaosExilesEachPlayersCreaturesAndUsesTheirModifiedPower() {
        addPlane(playerA, Planes.PLANE_GRAND_OSSUARY);
        addCard(Zone.BATTLEFIELD, playerA, "Grizzly Bears");
        addCard(Zone.BATTLEFIELD, playerB, "Hill Giant");
        addCounters(1, PhaseStep.UPKEEP, playerA, "Grizzly Bears", CounterType.P1P1, 1);
        ...
        assertPermanentCount(playerA, "Saproling Token", 3);
    ...
    public void registryExposesMetadata() {
        PlanarCardRegistry.Metadata metadata = PlanarCardRegistry.getMetadata(
                PlanarCardRegistry.getId(Planes.PLANE_GRAND_OSSUARY)
        );
