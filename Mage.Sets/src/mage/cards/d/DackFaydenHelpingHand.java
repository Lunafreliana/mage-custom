...
class DackFaydenHelpingHandEffect extends OneShotEffect {
    ...
    DackFaydenHelpingHandEffect() {
        ...
        this.staticText = "reveal cards from the top of your library until you reveal X creature cards, "
                ...
                + "then shuffle. They're goaded for the rest of the game. For each of those permanents, choose "
                + "a different opponent. Each opponent gains control of the permanent for which they were chosen";
    }
    ...
    public boolean apply(Game game, Ability source) {
        ...
        List<UUID> chosenOpponents = new ArrayList<>();
        for (Permanent permanent : permanents) {
            FilterOpponent filter = new FilterOpponent("different opponent");
            ...
            target.withTargetName("an opponent to gain control of " + permanent.getName());
            if (!controller.choose(outcome, target, source, game)) {
                continue;
            }
            chosenOpponents.add(target.getFirstTarget());
        }

        for (int i = 0; i < chosenOpponents.size(); i++) {
            Permanent permanent = permanents.get(i);
            ContinuousEffect effect = new GainControlTargetEffect(Duration.EndOfGame, chosenOpponents.get(i));
